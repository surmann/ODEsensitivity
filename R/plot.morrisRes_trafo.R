#' @title
#' Plotting the results of Morris SA for objects of class 
...
plot.morrisRes_trafo <- function(x, type = "sep", colors_pars = NULL, 
  ...
  if(type == "sep"){
    plotSep(x, pars, colors_pars = colors_pars, 
            common_title = main_title, legendPos = legendPos, ...)
  }
  
  # Trajectories:
  if(type == "trajec"){
    plotTrajectories(x, pars, colors_pars = colors_pars, 
