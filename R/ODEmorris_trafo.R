...
#' ordinary differential equations using Morris's elementary effects screening 
#' method and supports transformations of the adapted 
...
#'   function to transform the \code{n} times \code{z}-matrix of the adapted 
#'   \code{\link[deSolve]{ode}}-results for the \code{z > 1} output variables to
#'   \code{IR^n} (only needed, if \code{z > 1}), where \code{n} stands for the
...
#'   \code{\link[sensitivity]{morris}}-function which means a transformation has
#'   to be applied to the adapted \code{\link[deSolve]{ode}}-results used for 
#'   \code{\link[sensitivity]{morris}} (e.g. choosing only one output variable),
#'   so that this adapted model function returns a vector. This also means that 
#'   only one timepoint per can be analyzed per execution of
